...
PATIENT_ID = "SAMPLE-19019"  # Unique patient identifier (set by client)
...
if __name__ == "__main__":
    ...
    requests.put(
        upload_pre_signed_url_json['uploadUrl'],  # Pre-signed S3 URL
        headers=upload_pre_signed_url_json['headers'],  # Required headers
    ...
    site_sonar_response_json = requests.get(
        f"{DOMAIN}/v0/projects/{PROJECT_ID}/patients/{PATIENT_ID}/site-sonar",
        headers=headers
    ).json()
    ...
    # Extract list of sites from the response for further processing
